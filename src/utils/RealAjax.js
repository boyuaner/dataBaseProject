...
class Ajax {
  ...
  login(stuId, password) {
    ...
    return this.post(`${globalConfig.getAPIPath()}${globalConfig.login.validate}`, { stuId, password }, { headers });
  }
  register(stuId, password, stuTel, stuName) {
    const headers = { 'Content-Type': 'application/x-www-form-urlencoded' };
    return this.post(`${globalConfig.getAPIPath()}${globalConfig.login.register}`, { stuId, password, stuTel, stuName }, { headers });
  }
  /**
   *  封装CRUD相关操作
   *
  ...
  getActList(stuId) {
    const headers = { 'Content-Type': 'application/x-www-form-urlencoded' };
    return this.get(`${globalConfig.getAPIPath()}${globalConfig.myTable.actList}?stuId=` + stuId, { headers });
  }
  getDetail(actId) {
    const headers = { 'Content-Type': 'application/x-www-form-urlencoded' };
  }
